...
#include "detail/jsonrpc_query_merge.hpp"
#include "detail/jsonrpc_http_status.hpp"
#include "../logger.hpp"
...
public:
  ...
  void send_response_(const data_type& d, const output_handler_t& handler, boost::beast::http::status status)
  {
  ...
  {
    const auto status = detail::json_rpc_is_bad_gateway(d)
      ? boost::beast::http::status::forbidden
      : boost::beast::http::status::ok;

    if ( d.empty() )
      this->send_response_<boost::beast::http::empty_body>(d, handler, status);
    else
      this->send_response_<boost::beast::http::buffer_body>(d, handler, status);
  }

  void send_response(const output_handler_t& handler)
  {
    this->send_response_<boost::beast::http::empty_body>(data_type(), handler, boost::beast::http::status::ok);
  }
...
try
{
  if ( auto pconn = wconn.lock() )
  {
    if (auto msg = pconn->fetch() )
    {
      ...
      {
        static const jsonrpc_query_merge query_merge;
        auto pcontent = std::make_unique<data_type>(msg->body());
        if ( pcontent->empty() && !uv.encoded_query().empty() )
        {
          data_type query_body = query_merge.make_from_query(uv);
          if ( !query_body.empty() )
            pcontent = std::make_unique<data_type>(std::move(query_body));
        }
        else if ( !pcontent->empty() && !uv.encoded_query().empty() )
        {
          query_merge.merge_into_body(uv, pcontent.get());
        }
