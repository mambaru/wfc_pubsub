...
struct message
{
  ...
  /** Идентификатор получателя: 0 — публичное, иначе приватное сообщение для key */
  key_t key = 0;
  ...
  /** Приватное сообщение — доставляется только агенту с совпадающим key */
  static bool is_private(const message& m)
  {
    return m.key != 0;
  }

  /** Подходит ли сообщение получателю с заданным key агента */
  static bool for_recipient(const message& m, key_t recipient_key)
  {
