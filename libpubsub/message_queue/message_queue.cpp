...
struct f_deathtime_less
{
  ...
  bool operator()( const stored_message& m) const
  {
...
struct f_identity_equal
{
  ...
  bool operator()( const stored_message& msg) const
  {
...
struct f_modify_if_identity_equal
{
  ...
  void operator() (stored_message& msg)
  {
...
size_t message_queue::removed_count(bool reset) const
{
  size_t res = _removed;
  if (reset)
    _removed = 0 ;
...
stored_message::ptr message_queue::push( message&& m)
{
  return this->push( stored_message( std::move(m) ));
}

stored_message::ptr message_queue::push( const stored_message& m)
{
  time_t now = time(nullptr);
  return this->push( now, std::move(m) );
}

stored_message::ptr message_queue::push( time_t now,  message&& m)
{
  return this->push( now, stored_message( std::move(m) ) );
}


stored_message::ptr message_queue::push( time_t now, const stored_message& m)
{
  if ( m.is_prio() )
  ...
  switch( m.action() )
  {
    case actions::publish: publish_(now, m ); break;
    case actions::modify:  modify_(now, m ); break;
    case actions::update:  update_(now, m ); break;
    case actions::replace: replace_(now,m ); break;
    case actions::remove:  remove_(now, m ); break;
...
size_t message_queue::remove_death()
{
  return remove_death(time(nullptr));
}
...
/// @brief Удаляет сообщения время жизни которых закончилось относительно заданного времени
  ...
  */
size_t message_queue::remove_death(time_t now)
{
...
message_queue::queue_type::iterator message_queue::insert_( time_t now, const stored_message& m )
{
  // Находим позицию
...
void message_queue::publish_(time_t now, const stored_message& m)
{
...
void message_queue::update_(time_t now, const stored_message& m)
{
...
void message_queue::modify_(time_t now, const stored_message& m )
{
...
void message_queue::remove_( time_t now, const stored_message& m)
{
...
/// @brief
void message_queue::replace_(time_t now, const stored_message& m )
{
  remove_(now, m.get());
  publish_(now, std::move(m) );
}
