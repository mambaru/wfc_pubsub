...
namespace{
  ...
  UNIT(longpolling2, "")
  {
    ...
    fs = pubsub::fire_stat();
    lp.fire(&fs);
    t << equal<assert>(fs.hub_queue.counter_map, 0) << FAS_FL;
  ...
  UNIT(longpolling3, "")
  {
    ...
    t << equal<expect>(fs.hub_queue.describe_delay, 3) << FAS_FL;
    ...
    t << equal<expect>(fs.hub_queue.counter_map, 0) << FAS_FL;
