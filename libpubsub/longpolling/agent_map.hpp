...
public:
  ...
  // displaced — предыдущий waiter (вызвать вне lock с пустым списком).
  bool longpoll(const agent_params& params, const longpoll_hundler_t& handler, longpoll_hundler_t* displaced = nullptr);

  bool get_oid(const std::string& uuid, user_id_t* oid) const;

  size_t count_without_oid() const;

  // Отправить накопленные сообщения. pending_out — доставки вызвать вне lock.
  void fire(fire_stat *stat, std::vector<std::string>* channels, std::vector<pending_longpoll>* pending_out = nullptr);
