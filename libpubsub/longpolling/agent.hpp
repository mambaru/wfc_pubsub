...
struct pending_longpoll
{
  longpoll_hundler_t handler;
  topic_list_t topics;
};
...
public:
  ...
  // Если уже был ожидающий — возвращается в displaced (вызвать вне lock).
  longpoll_hundler_t longpoll(const longpoll_hundler_t& handler);
  ...
  // Если ready — заполняет pending (handler вызвать вне lock).
  bool fire(fire_stat *stat, pending_longpoll* pending);
  ...
  const agent_options& options() const;
