...
subscribe_manager::subscribe_manager(const longpolling_options& opt)
  ...
  , _inflight_subscribe(opt.inflight_timeout_s)
  , _inflight_describe(opt.inflight_timeout_s)
...
void subscribe_manager::unsubscribe_now(const std::string& channel)
{
  if ( channel.empty() )
    return;

  _wait_subscribe.erase(channel);

  for ( auto itr = _describe_delay.begin(); itr != _describe_delay.end(); )
  {
    if ( itr->second == channel )
      itr = _describe_delay.erase(itr);
    else
      ++itr;
  }

  _counter_map.erase(channel);
...
bool subscribe_manager::pop_for_subscribe(std::vector<std::string>* cl, size_t limit)
{
  ...
  if ( size_t timed_out = _inflight_subscribe.pop_outdated(&_wait_subscribe) )
  {
    // Такого не должно происходиить, поэтому это ошибка, а не предупреждение
    LONGPOLL_LOG_ERROR("Таймауты на subscribe от хаба в количестве: " << timed_out << ". Ожидается повторный запрос.");
  }
  return this->pop_for_(&_wait_subscribe, &_inflight_subscribe, cl, limit);
}

bool subscribe_manager::pop_for_describe(std::vector<std::string>* cl, size_t limit)
{
  if ( size_t timed_out = _inflight_describe.pop_outdated(&_wait_describe) )
  {
    // Такого не должно происходиить, поэтому это ошибка, а не предупреждение
    LONGPOLL_LOG_ERROR("Таймауты на describe от хаба в количестве: " << timed_out << ". Ожидается повторный запрос.");
  }
  ...
  return this->pop_for_(&_wait_describe, &_inflight_describe,  cl, limit);
}

void subscribe_manager::confirm_for_subscribe( const std::vector<std::string>& cl)
{
  _inflight_subscribe.ready(cl);
}
...
void subscribe_manager::clear()
{
  ...
  _inflight_subscribe.clear();
  _inflight_describe.clear();
}
...
hub_queue_stat subscribe_manager::stat() const
{
  hub_queue_stat sstat;
  ...
  sstat.inflight_subscribe = _inflight_subscribe.size();
  sstat.inflight_describe = _inflight_describe.size();
