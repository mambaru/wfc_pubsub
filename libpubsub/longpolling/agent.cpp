...
bool agent::update(const agent_options& opt)
{
  ...
  if ( _opt.oid != 0 && opt.oid != 0 && _opt.oid != opt.oid )
    return false;
...
const agent_options& agent::options() const
{
  return _opt;
}
...
{
  if ( !message::for_recipient(m, _opt.key) )
    return false;
...
longpoll_hundler_t agent::longpoll(const longpoll_hundler_t& handler)
{
  longpoll_hundler_t displaced = std::move(_longpoll_hundler);
  _longpoll_hundler = handler;
  _deathtime = time(nullptr) + _opt.agent_lifetime;
  return displaced;
}

bool agent::fire(fire_stat *stat, pending_longpoll* pending)
{
  ...
  if ( ready )
  {
    ...
    if ( pending != nullptr )
    {
      pending->handler = std::move(_longpoll_hundler);
      pending->topics = topic::copy_list(_wait_list); // копия: _wait_list чистит confirm()
    }
    else
    {
